...
[Collegiate Cyber Defense Competition](https://www.nationalccdc.org/) (CCDC)
...
[Nick O'Brien](https://github.com/cictrone) and I decided to look into the
...
![Ping command. Ping succeeds to 8.8.8.8, fails to "8.8.8.8; whoami" with an "Invalid syntax" error.](/assets/cve-2019-1576/ping.png)
...
![Command: "less mp-global userinfo.xml". Lists the contents of that file, which is a device-specific user listing.](/assets/cve-2019-1576/less_1.png)
...
![Same command as last figure, but "!bash" is entered which then opens a bash shell. id is run showing the session is for the "admin" user. uname -a is run showing the kernel version as Linux 3.10.88.](/assets/cve-2019-1576/less_2.png)

This vulnerability [was given a 6.5 score on the CVSS 2.0 scale](https://nvd.nist.gov/vuln/detail/CVE-2019-1576)
...
![Part of the Binary Ninja control flow graph showing the default parsing call if the command isn't detected as invalid](/assets/cve-2019-1576/cfg_original.png)

![The same part showing "Command not available" is passed to a call to "error" for the above commands](/assets/cve-2019-1576/cfg_fix.png)
